...
export const AnalyticsModal: React.FC<AnalyticsModalProps> = ({
  ...
  // ── SDG Chart Data ──
  const sdgCounts: Record<string, number> = { SDG6: 0, SDG13: 0, SDG15: 0, SDG14: 0, SDG7: 0, SDG11: 0 };
  ...
  if (sdgReport && Array.isArray(sdgReport.observations_by_sdg)) {
    ...
    geoPoints.forEach(p => {
      p.sdgTags.forEach(sdg => {
        if (sdgCounts[sdg.code] !== undefined) sdgCounts[sdg.code] += 1;
      });
    });
  }

  const sdgChartData = [
    { name: 'SDG 6 (Water)', count: sdgCounts.SDG6 || 0, color: '#36AAE0' },
    { name: 'SDG 13 (Climate)', count: sdgCounts.SDG13 || 0, color: '#326B32' },
    { name: 'SDG 15 (Land)', count: sdgCounts.SDG15 || 0, color: '#57B039' },
    { name: 'SDG 14 (Coastal)', count: sdgCounts.SDG14 || 0, color: '#0A97D9' },
    { name: 'SDG 11 (Urban)', count: sdgCounts.SDG11 || 0, color: '#FD9D24' },
  ];

  // ── Governorate Chart Data ──
  const govMap: Record<string, { count: number; totalNdvi: number }> = {};
  geoPoints.forEach(p => {
    const gov = p.governorate || 'Unspecified';
    if (!govMap[gov]) govMap[gov] = { count: 0, totalNdvi: 0 };
    govMap[gov].count += 1;
  ...
  const govChartData = Object.keys(govMap).map(gov => ({
    ...
    avgNdvi: govMap[gov].count > 0 ? Number((govMap[gov].totalNdvi / govMap[gov].count).toFixed(2)) : 0,
  }));

  // ── Layer Chart Data ──
  const layerChartData = layers.map(l => ({
    name: lang === 'ar' ? l.titleAr.split(' ')[0] : l.titleEn.split(' ')[0],
    value: geoPoints.filter(p => p.layerId === l.id).length || 0,
    color: l.color,
  })).filter(l => l.value > 0);
  ...
  const verifiedPoints = backendSummary ? (backendSummary.approved_observations || 0) : geoPoints.filter(p => p.verificationStatus === 'verified').length;
  const verifiedPercent = totalPoints > 0 ? Math.round((verifiedPoints / totalPoints) * 100) : 0;

  const avgNdvi = backendSummary ? backendSummary.avg_ndvi : (() => {
    const list = geoPoints.filter(p => typeof p.metrics?.ndvi === 'number');
    if (!list.length) return 0;
    const sum = list.reduce((a, p) => a + (p.metrics!.ndvi || 0), 0);
    return Number((sum / list.length).toFixed(2));
  })();

  const criticalHotspots = backendSummary ? backendSummary.critical_hotspots : geoPoints.filter(p => p.threatLevel === 'critical').length;
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto select-none">
      <div className="bg-white rounded-xl shadow-2xl border border-[#D1DCE5] w-full max-w-4xl overflow-hidden my-6">
        ...
        <div className="bg-[#006BB2] text-white p-4 flex items-center justify-between border-b border-[#005794]">
          ...
          <button onClick={onClose} className="p-1 hover:bg-white/10 rounded-full transition-colors text-white cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-6 max-h-[82vh] overflow-y-auto bg-slate-50">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className="bg-white p-3.5 rounded-lg border border-[#D1DCE5] shadow-xs">
              ...
              <div className="text-2xl font-extrabold text-[#009600] font-coord mt-1">{avgNdvi}</div>
            ...
            <div className="bg-white p-3.5 rounded-lg border border-[#D1DCE5] shadow-xs">
              ...
              <div className="text-2xl font-extrabold text-rose-600 font-coord mt-1">{criticalHotspots}</div>
            ...
            <div className="bg-white p-4 rounded-xl border border-[#D1DCE5] shadow-xs">
              ...
              <div className="h-56">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={sdgChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
